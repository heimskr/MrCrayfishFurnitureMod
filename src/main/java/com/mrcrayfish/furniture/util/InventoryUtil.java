...
public class InventoryUtil {
     ...
     */
    public static ListNBT writeInventoryToNBT(ISimpleInventory inventory, ListNBT nbt) {
        if (nbt != null)
     ...
     */
    public static ItemStack[] readInventoryFromNBT(ISimpleInventory inventory, ListNBT nbt) {
        ...
        if (nbt != null)
            for (int i = 0; i < nbt.size(); ++i) {
                ...
                if (slot >= 0 && slot < items.length)
                    items[slot] = ItemStack.read(itemTag);
            }
        return items;
    }
}
